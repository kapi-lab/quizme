...
### 2.2 用户画像升级：方向与兴趣（Direction & Interests）

现有画像只有 level（Junior/Mid/Senior/Staff+）。新增：

```ts
interface UserProfile {
  direction?: string;   // 自由文本，一句话描述方向/岗位，如"做推荐系统的后端，最近在补 K8s"
  interests: string[];  // 感兴趣的领域标签（自由输入，引导时给常见项作快捷选择）
  avoids: string[];     // 不感兴趣/暂不想学的领域标签
}
```

**不用枚举的原因**：direction 的唯一消费方是提取/出题 prompt（LLM），不是代码分支——自由文本对模型的信息量远大于固定枚举；强行归入 8 个大方向反而丢失"推荐系统 + 正在学 K8s"这类真正影响出题的细节。

**填充途径（两条，都是轻交互）**：

1. **首次引导**：一句话方向（可跳过）+ 从常见领域标签中多选感兴趣项（也可自由输入）；不感兴趣项可选填。
2. **答题中持续修正**：卡片背面提供"这个方向我不感兴趣"反馈，一键把该卡领域标签加入 `avoids`；后续提取与组卡即时生效。画像不是一次性问卷，而是越用越准。
...
分配规则（在 KP 提取阶段由模型结合 direction 自由文本与兴趣标签判定，用户可手动调）：

- 领域命中 `interests`（或与 direction 描述强相关）→ 目标 D3（想深耕的方向学到原理）
- 未命中兴趣但与用户工作相邻 → 目标 D2（会用、能选型即可）
- 远域但工程师应知 → 目标 D1（知道概念即可）
- 领域命中 `avoids` → 默认不出题；若知识点极重要（模型判定）降为 D1 偶尔出现
