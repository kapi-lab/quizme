...
QuizMe 读取 `~/.claude/projects/` 中最近一次 Claude Code 会话（跨项目全局最新）。这样用户在等待间隙从任意目录启动 QuizMe 都能立即拿到最近上下文。如果 `~/.claude/projects/` 不存在、不可读或无法解析，QuizMe 应清晰报错，不静默 fallback 到其他模式。
...
QuizMe 也需要说明：默认会扫描用户级 `~/.claude/projects/` 找最近一次会话，如果读取失败会报错。
...
- 首次配置
- 直接读取用户级 `~/.claude/projects/` 中最新会话，失败时报错
- 显式 `--repo` 模式
- 主题模式
...
- 题目生成和 `why` 调用 `claude -p` 时禁用全部 agent tool（`--safe-mode` + `--tools ""`）；上下文由 QuizMe 本地摘要后写入 prompt，模型不再访问文件系统。
- 不做初始 placement quiz。
